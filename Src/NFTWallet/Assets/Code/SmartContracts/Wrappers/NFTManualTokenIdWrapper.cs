...
public class NFTManualTokenIdWrapper
{
    ...
    public static async Task<string> DeployNFTContractAsync(StratisUnityManager stratisUnityManager, string name, string symbol, bool ownerOnlyMinting)
    {
        ...
        string txId = await stratisUnityManager.SendCreateContractTransactionAsync(WhitelistedContracts.NFTManualTokenId.ByteCode, constructorParameter.ToArray(), 0);
    ...
    public NFTManualTokenIdWrapper(StratisUnityManager stratisUnityManager, string contractAddress)
    {
    ...
    /// <remarks>Normal call. Use returned txId to get receipt in order to get return value once transaction is mined. Return value is of <c>bool</c> type.</remarks>
    public async Task<string> MintAsync(string addrTo, UInt256 tokenId, string uri)
    {
        List<string> parameters = new List<string>()
        {
            $"{(int)MethodParameterDataType.Address}#{addrTo}",
    ...
    /// <summary>Mints new tokens</summary>
    /// <remarks>Normal call. Use returned txId to get receipt in order to get return value once transaction is mined. Return value is of <c>bool</c> type.</remarks>
    public async Task<string> SafeMintAsync(string addrTo, UInt256 tokenId, string uri, byte[] data)
    {
        var bytesString = BitConverter.ToString(data).Replace("-", "");
